...
from .airtable import tools as at_tools
...
class Cruder:
    @staticmethod
    def _fmt_col_names(column: str, prefix: str, style: str, abbrs: list):
        """Format Column Names"""
        style = style.lower()
        col_name = ""
        if prefix:
    ...
    @staticmethod
    def convert_df(df: pd.DataFrame, output: str = "records") -> list:
        """
    ...
    @staticmethod
    def crud(data: list, dest: str, **kwargs) -> None:
        """
        ...
        """  # noqa: E501
        if dest == "airtable":
            Cruder._at_crud(data, **kwargs)
        elif dest == "sheets":
            Cruder._sh_crud(data, **kwargs)

    @staticmethod
    def _at_crud(records: list, **kwargs) -> None:
        """
        ...
        existing_records = table.get_records()

        # Get linked tables
        for link in links:
            linked_table = base.get_table(link["table"])
            link["table"] = linked_table.get_records()
            records = at_tools.link_tables(
        ...
        if existing_records:
            ...
            for i, existing_record in enumerate(existing_records):
                hashable_keys = at_tools.get_primary_keys_as_hashable(
                    existing_record, primary_keys
                )
                if hashable_keys:
                    existing_records_indices_by_primary_key[hashable_keys] = i

            for record in records:

                # turn array values into lists
                if arrays:
                    record = at_tools.graft_fields(record, arrays)
                ...
                if existing_index is not None:
                    ...
                    if overrides:
                        record = at_tools.override_record(
                            record, existing_record, overrides
                        )

                    # filter records to only keep new data
                    record = at_tools.filter_record(record, existing_record)
            ...
            # create new records
            table.post_records(post_records)
            # update existing records
            table.update_records(update_records)
            ...
            if len(dead_indices) > 0:
                ...
                for index in dead_indices:
                    ...
                    record_name = dead_record["fields"].get(
                        table.primary_field_name
                    )
                    # get record's first field
                    if not record_name:
                        field_names = list(dead_record["fields"].keys())
                        if field_names:
                            record_name = dead_record["fields"].get(
                                field_names[0]
                            )
                        else:
                            record_name = dead_record["id"]

                    # delete record
                    if mode == "overwrite":
                        table.delete_record(dead_record, message=record_name)
                    # flag manual deletion via 'Delete' checkbox field
                        ...
                        if not dead_record["fields"].get(del_field):
                            ...
                            table.update_record(record, message=record_name)
        ...
        # If no records in table
            ...
            if arrays:
                # turn array values into lists
                records = [
                    at_tools.graft_fields(record, arrays) for record in records
                ]
